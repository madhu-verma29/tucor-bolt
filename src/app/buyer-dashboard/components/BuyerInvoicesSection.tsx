...
import { buyerApi, type BuyerPayment } from '@/lib/buyer-api';
...
export default function BuyerInvoicesSection() {
  const [invoiceData,setInvoiceData]=useState<(BuyerPayment & {description:string;period:string;type:string})[]>([]); useEffect(()=>{buyerApi.payments().then(ps=>setInvoiceData(ps.filter(p=>p.status==='Settled'&&!!p.invoiceNumber).map(p=>({...p,description:'UCO Procurement Invoice',period:p.settledDate?new Date(p.settledDate).toLocaleString('en-IN',{month:'short',year:'numeric'}):'—',type:'Procurement Invoice'})))).catch(()=>setInvoiceData([]));},[]);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Download and manage settled TUCOR procurement invoices
          </p>
        </div>
        <button
          onClick={() => toast.success('Invoice export requires generated invoice documents')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'This Month', value: invoiceData.filter((i) => i.period === new Date().toLocaleString('en-IN',{month:'short',year:'numeric'})).length, sub: new Date().toLocaleString('en-IN',{month:'short',year:'numeric'}), color: 'text-info', bg: 'bg-info-bg', icon: Receipt },
        ].map((stat) => {
              ...
              {filtered.map((inv) => {
                ...
                return (
                  <tr key={`inv-${inv.id}`} className="hover:bg-muted/30 transition-colors duration-100">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => toast.info('Invoice PDF generation will be available after invoice-document service integration')}
